...
from utils import open_file_write
...
class Picross(object):
    ...
    def save_grid(self, model=Model.MODEL_AYOUB, solver=Solver.PYCOSAT_SOLVER, output=None):
        ...
        output = output or (self.file_path.split('.')[0] + '.GRID')
        f = open_file_write(output)
    ...
    def save_dimacs(self, model=Model.MODEL_AYOUB, output=None):
        ...
        output = output or (self.file_path.split('.')[0] + '.DIMACS')
        f = open_file_write(output)
        for c in self.comments:
            f.write('{}\n'.format(c))
